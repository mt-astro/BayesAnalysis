...
- Very good. (You were at a disadvantage, having missed a couple of meetings devoted mainly to coding.)  

- When plotting something that is continuous or piecewise continuous, use `type = "l"` unless you intend to discuss the effects of undersampling.  

Ex 3.00: 5/10.  

Ex 3.01: 12/10. Thoughtful and interesting.     

Ex 3.1:  9/10.  I like it that you used `type = "b"` here, because it shows the artifacts of sampling y=1/x, but in general, when plotting something that is continuous or piecewise continuous, one should use `type = "l"`. Good variable names.   

Ex 3.2a: 5/10.  Your `recip()` works, but only for scalar `x`. Also, using `append()` to populate a vector in a for-loop is horribly slow. If `x` were length 10^6^ it would take several minutes to run---see the little test below using `system.time()` for x length 10^5^. In any computer language one should try to try to allocate storage _before_ entering the loop that will populate it, in order that the storage be contiguous in memory. That isn't always possible, but it is possible most of the time. See the solutions for betteR ways to do this problem, because they do not require loops. 

Here is a chunk that compares the `append()` method with the create-the vector first method.  


```{r, collapse=FALSE}
N <- 100000 # one hundred thousand

# The following took 13.5 seconds of system time on my machine
system.time( {
  y <- c()
  for (i in 1:N) y <- append(y, i^2)
} )

# whereas the following took less than 10^{-3} seconds!
system.time( {
  y <- rep(0, N)
...
Ex 3.3:  9/10.  The plot is attractive, but it would not be acceptable in a publication. As I noted above, when the underlying mathematical object is continuous, or piecewise continuous, one should use a line plot rather than a point plot unless the discussion includes the effects of undersampling.  

Ex 3.4:  4/5.  `align="c"` is generally better, I think, especially when a column heading is more than a few letters wide. 

Ex 3.5:  5/10. Good, but you were asked to use _inline code_ to display results. See the solutions for what is meant by that.   
