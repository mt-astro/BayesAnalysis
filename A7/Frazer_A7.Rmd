...
f <- function(x) {7*x^8*(1-x)^18 + 3*x^18*(1-x)^8} # posterior

Ns <- 2000 # numer of samples  
SD <- 0.2  # sd of normal proposal distn
x <- numeric(Ns) # allocate storage
x[1] <- runif(1) # starting point
for (i in 2:Ns) {
  nowat <- x[i-1]
  move <- rnorm(1, mean=0, sd=SD)
  prop <- nowat + move  # proposal
  if (prop<0 || prop>1) {
    x[i] <- nowat
    next
  } 
  ...
  if (fp >= fn) {
    ...
    x[i] <- ifelse(runif(1) < fp/fn, prop, nowat) 
  }
}
...
fv <- function(a,b) a*b/(a + b)^2/(a + b + 1)
theoryVar <- fv(a1, b1) + fv(a2, b2)
