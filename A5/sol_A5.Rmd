...
# Solution 5.0  
(Notice that this paragraph uses footnotes for the definitions. When you click on a footnote in the html you are taken there, and clicking on the curved arrow will take you back.) The technical terms I noticed are: 
frequentist, 
_F_-ratio, 
Bayes rule, 
prior (distribution), 
...
row attribute, 
column attribute, 
hit rate (for a diagnostic test), 
false alarm rate, 
base rate, 
row marginal, 
column marginal, 
...
marginal likelihood, 
prior predictive, 
evidence,^[The number you get when you substitute the actual data values into the prior predictive is called the evidence.] 
data order invariance, 
independence,^[Spatiotemporal data points are seldom independent, which is why time series analysis and spatial analysis require special techniques] 
bias, 
categorical (data), 
Bernoulli distribution, 
sample size, 
candidate value (of a parameter), 
...
## Ex 5.0.5(b)  
Glance at the Wikipedia entry for [Sensitivity and specificity](https://en.wikipedia.org/wiki/Sensitivity_and_specificity), and page 103 of our text, then group the following terms into exactly four classes, such that the terms in each class have the same meaning: specificity, fall-out, true negative rate, sensitivity, false alarm rate, true positive rate, hit rate, false negative rate, false positive rate. It is not necessary to name the classes; just group the terms.
...
# Solution 5.1.1  
It is easy to code a solution to this problem in a few short lines, but not very easy to get it right. I did it twice because my first version did not seem as clear as it could be. John Chambers, one of the creators of R, talks about the _Prime Directive_ for programming, which is that code should be _understandable_ and _trustworthy_. Both versions use a probability matrix, with the prior and posterior as vectors, because that approach generalizes nicely to more complicated problems. 

## Version 1  
The numerical row and column indices in this version don't make for very clear code.   
...
ppD <- function(tr=1, tpr, fpr, prior) {
  ...
  fnr <- 1 - tpr # sum rule: tpr + fnr = 1
  tnr <- 1 - fpr # sum rule: fpr + tnr = 1
...
## Now do the problem on page 103. (The data assignments are 
## for inline code in the following narrative.)
tr <- 1
tpr <- 0.99
fpr <- 0.05
prior <- 0.001
...
For this problem we considered a test with true positive rate `r tpr*100`% and false positive rate `r fpr*100`% for a disease with prior probability `r prior*100`%. Given a `r ifelse(tr, "positive", "negative")` test result, the posterior probability of disease is `r signif(post1*100, 3)`%. This result is in agreement with the result on page 104 of our text, so my code is at least not obviously wrong.   

## Version 2  

This time I used row and column _names_ to index the probability matrix. The new function `ppD2()` also does some argument checking and self-checking, which make it longer. I think it is clearer than the function used above, but you be the judge.    

```{r ex5.1.1b}
ppD2 <- function(test="pos", tpr, fpr, prior) {
  # test: the test result, either "pos" or "neg"
  # tpr: true positive rate (sensitivity) of the test
  # fpr: false positive rate (1-specificity)
  # prior: prior probability of the disease
  stopifnot(test %in% c("pos", "neg"),
            0 < tpr, tpr < 1, 0 < fpr, fpr < 1) 
  
  ## false negative rate, true negative rate
  fnr <- 1 - tpr # sum rule: tpr + fnr = 1
  tnr <- 1 - fpr # sum rule: fpr + tnr = 1
  
  ## matrix of conditional probabilities Pr(Test|Disease)
  pTgD <- matrix(rep(0,4), nrow=2)
  rownames(pTgD) <- c("pos", "neg")        # test result
  colnames(pTgD) <- c("present", "absent") # disease 
  pTgD["pos", "present"] <- tpr     
  pTgD["pos", "absent" ] <- fpr     
  pTgD["neg", "present"] <- fnr     
  pTgD["neg", "absent" ] <- tnr     
  
  Prior <- c(present=prior, absent=1-prior)  # vector prior
  Post  <- c(present=NA   , absent=NA     )  # vector posterior
  
  evidence <- pTgD[test,"present"]*Prior["present"] +
              pTgD[test,"absent" ]*Prior["absent" ]
  
  Post["present"] <- pTgD[test,"present"]*Prior["present"]/evidence
  Post["absent" ] <- pTgD[test,"absent" ]*Prior["absent" ]/evidence
  
  stopifnot(all.equal(1, sum(Post))) # check
  return(Post["present"])
}
...
fpr <- 0.05
prior <- 0.001
post11 <- ppD2(test=test, tpr=tpr, fpr=fpr, prior=prior) 
```  
Using the new function `ppD2()` the posterior probability of disease given one positive test is `r signif(100*post11,3)`%, the same value we obtained above.    
...
(See my note above, in the solution to Ex 5.1.1, regarding how to code this better next time.)    
...
ppDm <- function(tr=1, tpr, fpr, prior) {
  ## argument list
  # tr: vector of 1's and 0's. 1's indicate positive test results
  # tpr: true  positive rate(s)
  # fpr: false positive rate(s)
  # prior: prior probability of disease
  
  ## Check arguments. Not all bads will be caught, I fear  
  ltr <- length(tr) # used below
  stopifnot(ltr >= 1,
            length(tpr)==ltr || length(tpr)==1,
            length(fpr)==ltr || length(fpr)==1)
  
  ## expand tpr, fpr if needed
  if (ltr > 1) {
...
TR <- ifelse(tr, "+ve", "-ve")
df <- data.frame("Test result"=TR, TPR=tpr, FPR=fpr, TNR=1-fpr, FNR=1-tpr)
```  

For this problem we considered a battery of five tests with results as given in the following table. The prior probability of disease is `r prior*100`% and the posterior probabilty of disease is `r post*100`%.  

Note: In order to have a space in a column name it seems to be necessary to use kable( )'s `col.names` argument. 

```{r }
col.names <- c("Test result", "TPR", "FPR", "TNR", "FNR")
kable(df, col.names=col.names,
      caption=paste0("**Table 5.1.3.** A battery of five tests with 
                     various true positive rates and false positive 
                     rates. The posterior probability of disease is ",    
                     signif(post*100, 4), "%."))
```  

# Ex 5.1.4  
(Not in text) (5 points)  
...
As in Ex 5.1.1, the test has true positive rate `r tpr*100`% and false positive rate `r fpr*100`% for a disease with prior probability `r prior*100`%. Given a `r ifelse(tr, "positive", "negative")` test result, the posterior probability of disease, calculated using our new function `ppDm()`, is `r signif(post3*100, 3)`%, in agreement with Ex 5.1.1.  

```{r ex5.1.4b}
tr <- c(1, 0)
post4 <- ppDm(tr=tr, tpr=tpr, fpr=fpr, prior=prior)    
```  
Using our new function `ppDm()`, the probability of disease given one positive test result and one negative test result is `r signif(post4*100, 3)`%, in agreement with Ex 5.1.2.    
...
```{r ex6.5A2, fig.asp=0.5, fig.cap="**Figure 5.6A.** Testing `dmb()` our home-made version of `dbeta()`. The left panel graphs `dmb()` against `dbeta()` and the right panel shows the residuals. As the residuals are on the order of 10^-15^, our `dmb()` does very well---for these shape parameters."}  
...
In retrospect, doing a fit and looking at residuals from the fit was a dumb idea because `dmb()` could easily be wrong by a constant factor. For example, if all the values returned by `dmb()` were twice as large as they should be, the fit would still be excellent (tiny residuals) but the slope of the left panel would be 2 instead of 1, an oddity that might never be noticed because `plot()` automatically adjusts the axis scales. A better idea is to plot both `dmb(x)` and `dbeta(x)` versus `x` on the same set of axes and then make a histogram of the differences  `dmb(x) - dbeta(x)`. Here it is:  

```{r ex6.5B, fig.asp=0.5, fig.cap="**Figure 5.6B.** Testing `dmb()` our home-made version of `dbeta()`. The left panel graphs `dmb()` and `dbeta()` and the right panel shows the differences. As the differences are on the order of 10^-5^, our `dmb()` does very well---for these shape parameters."}  
par(mgp=c(2.5, 1, 0), mar=c(4.5, 4, 1, 1), mfrow=c(1, 2))
...
## left panel
plot(x, y1, type="l", bty="l", col="red", lwd=3,
     panel.first=grid(),
     xlab=expression(theta),
     ylab=expression("dmb("*theta*") and dbeta("*theta*")"))

lines(x, y2,  type="l", bty="l", col="black")

leg1 <- paste0("dmb("  , a, ",", b, ")")
leg2 <- paste0("dbeta(", a, ",", b, ")")

legend("bottomleft", inset=0.05, bty="n",
       lty=c(1, 1), col=c("black", "red"),
       lwd=c(1, 3), legend=c(leg1, leg2)
       )

## right panel
hist(dy, main="", xlab=expression("dmb("*theta*") - dbeta("*theta*")"))
```  
